<?php
require('./config/config.php');
require('./src/Models/Client.php');
require('./src/Models/Card.php');

$query = new QueryManager($pdo);
$cards = $query->getCardsWithClientDetails();


?>


<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <script src="https://cdn.jsdelivr.net/npm/qrcodejs2@1.0.0/qrcode.min.js"></script>
    <script src="https://cdn.tailwindcss.com"></script>

    <title>Cards</title>
</head>

<body class="bg-violet-100">
    <section class="flex ">
        <!-- side barre --->
        <div>
            <?php require('./views/component/SideBarAg.php'); ?>
        </div>

        <div class="px-20 py-10 flex flex-col gap-12 ml-[15%] w-[85%] ">
            <!--- My profil > MY card ---->
            <div>
                <span class="text-black text-opacity-60 text-[22px] font-normal">My Profile ></span>
                <span class="text-black text-[22px] font-normal">Card</span>
            </div>

            <div class="flex flex-col gap-4">
                <div class="w-[1100px] border-b-2 border-blue-900">
                    <h3 class="font-bold text-xl  text-blue-900">All Cards</h3>
                </div>
            </div>
            <!-- Barre de recherche -->
            <div class="flex mb-4">
                <input type="text" placeholder="Search..." class="p-2 border border-gray-300 rounded-l w-[1000px] focus:outline-none">
                <button class="bg-blue-900 text-white p-2 rounded-r">Search</button>
            </div>
            <!-- Tableau avec boutons d'action -->
            <div class="bg-white w-[1100px]  shadow-md rounded-lg ">
                <table class="w-full border-collapse w-[600px]">
                    <thead>
                        <tr class="bg-blue-900 text-white">
                            <th class="py-2 px-4">First Name</th>
                            <th class="py-2 px-4">Last Name</th>
                            <th class="py-2 px-4">Card Number</th>
                            <th class="py-2 px-4">Start Date</th>
                            <th class="py-2 px-4">End Date</th>
                            <th class="py-2 px-4">Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($cards as $card) : ?>
                            <tr class="hover:bg-blue-100 w-[600px]">
                                <td class="py-2 px-4"><?= $card['FirstName'] ?></td>
                                <td class="py-2 px-4"><?= $card['LastName'] ?></td>
                                <td class="py-2 px-4"><?= $card['CardNumber'] ?></td>
                                <td class="py-2 px-4"><?= $card['Start_Date'] ?></td>
                                <td class="py-2 px-4"><?= $card['End_Date'] ?></td>
                                <td class="py-2 px-4"><?= $card['Statut'] ?></td>
                                <td class="flex py-2 px-4">
                                    <button class="bg-green-500 text-white px-4 py-2 rounded mr-2" onclick="openUpdatePopup(
                        '<?= $card['CardID'] ?>',
                        '<?= $card['FirstName'] ?>',
                        '<?= $card['LastName'] ?>',
                        '<?= $card['CardNumber'] ?>',
                        '<?= $card['Start_Date'] ?>',
                        '<?= $card['End_Date'] ?>',
                        '<?= $card['Statut'] ?>',
                    )">Update</button>
                                    <button class="bg-red-500 text-white px-4 py-2 rounded" onclick="openDeletePopup('<?= $card['CardID'] ?>')">Delete</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>

                    </tbody>
                </table>

            </div>
            <div id="updatePopupAg" class="hidden popup bg-white absolute right-[20%] left-[30%] w-[600px] top-[20%] shadow-md rounded">
                <div class="popup-content flex flex-col items-center">
                    <div class="flex justify-end w-full px-4 py-2">
                        <span class="close text-3xl cursor-pointer" onclick="closeUpdatePopup()">&times;</span>
                    </div>
                    <h2>Update Card</h2>
                    <form class="flex flex-col gap-2 p-6" method="post" action="">
                        <input type="hidden" name="cardId" id="updateCardId">
                        <div class="flex">
                            <label class="flex w-[150px]" for="updateFirstName">First Name:</label>
                            <input class=" px-2 h-[30px] border rounded-[10px] border-black border-opacity-25" type="text" name="firstName" id="updateFirstName" required>
                        </div>
                        <div class="flex">
                            <label class="flex w-[150px]" for="updateLastName">Last Name:</label>
                            <input class="px-2 h-[30px] border rounded-[10px] border-black border-opacity-25" type="text" name="lastName" id="updateLastName" required>
                        </div>

                        <div class="flex">
                            <label class="flex w-[150px]" for="updateCardNumber">Card Number:</label>
                            <input class="px-2 h-[30px] border rounded-[10px] border-black border-opacity-25" type="number" name="cardNumber" id="updateCardNumber">
                        </div>
                        <div class="flex">
                            <label class="flex w-[150px]" for="updateStartDate">Start Date:</label>
                            <input class="px-2 h-[30px] border rounded-[10px] border-black border-opacity-25" type="date" name="startDate" id="updateStartDate">
                        </div>
                        <div class="flex">
                            <label class="flex w-[150px]" for="updateEndDate">End Date:</label>
                            <input class="px-2 h-[30px] border rounded-[10px] border-black border-opacity-25" type="date" name="endDate" id="updateEndDate">
                        </div>
                        <div class="flex">
                            <label class="flex w-[150px]" for="updateStatus">Status:</label>
                            <input class="px-2 h-[30px] border rounded-[10px] border-black border-opacity-25" type="text" name="status" id="updateStatus">
                        </div>



                        <button class="my-6 btnSubmit" name="submit" type="submit">Update</button>
                    </form>
                </div>
            </div>

            <div id="deletePopup" class="deletePopup bg-white absolute right-[20%] left-[30%] w-[400px] top-[20%] shadow-md rounded" style="display: none;">
                <div class="popup-content flex flex-col items-center">
                    <span class="close flex justify-end w-full px-4 py-2 text-3xl cursor-pointer" onclick="closeDeletePopup()">&times;</span>
                    <h2>Delete Card</h2>
                    <p>Are you sure you want to delete this card?</p>
                    <form class="flex flex-col gap-2 p-6" method="post" action="">
                        <input type="hidden" name="deleteCardId" id="deleteCardId">
                        <button class="my-6 btnSubmit bg-red" type="submit">Delete</button>
                    </form>
                </div>
            </div>
        </div>

    </section>
</body>

</html>


<script>
    function openDeletePopup(cardId) {
        var popup = document.getElementById("deletePopup");
        popup.style.display = "block";
        document.getElementById("deleteCardId").value = cardId;
    }

    function closeDeletePopup() {
        var popup = document.getElementById("deletePopup");
        document.getElementById("deleteCardId").value = "";
        popup.style.display = "none";
    }

    function openUpdatePopup(cardId, firstName, lastName, cardNumber, startDate, endDate, status) {
        var popup = document.getElementById("updatePopupAg");
        popup.style.display = "block";
        document.getElementById("updateCardId").value = cardId;
        document.getElementById("updateFirstName").value = firstName;
        document.getElementById("updateLastName").value = lastName;
        document.getElementById("updateCardNumber").value = cardNumber;
        document.getElementById("updateStartDate").value = startDate;
        document.getElementById("updateEndDate").value = endDate;
        document.getElementById("updateStatus").value = status;
    }

    function closeUpdatePopup() {
        var popup = document.getElementById("updatePopupAg");
        document.getElementById("updateCardId").value = "";
        document.getElementById("updateFirstName").value = "";
        document.getElementById("updateLastName").value = "";
        document.getElementById("updateCardNumber").value = "";
        document.getElementById("updateStartDate").value = "";
        document.getElementById("updateEndDate").value = "";
        document.getElementById("updateStatus").value = "";
        popup.style.display = "none";

    }
</script>

<?php
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['submit'])) {
        $cardDetails = $card->getCard($pdo, $_POST['cardId']);
        $firstName = isset($_POST['firstName']) ? $_POST['firstName'] : $cardDetails['FirstName'];
        $lastName = isset($_POST['lastName']) ? $_POST['lastName'] : $cardDetails['LastName'];
        $cardNumber = isset($_POST['cardNumber']) ? $_POST['cardNumber'] : $cardDetails['cardNumber'];
        $startDate = isset($_POST['startDate']) ? $_POST['startDate'] : $cardDetails['startDate'];
        $endDate = isset($_POST['endDate']) ? $_POST['endDate'] : $cardDetails['Phone'];
        $status = isset($_POST['status']) ? $_POST['status'] : $cardDetails['statut'];
        $card = new Card($cardNumber, $startDate, $endDate, $status, $card['AgentID'], $card['ClientID'], $card['SubscriptionID']);
        $success = $card->updateCard($pdo, $_POST['cardId']);
        if ($success) {
            echo "Updated";
        } else {
            echo "Update failed.";
        }
    }


    if (isset($_POST['deleteCardId'])) {
        // Delete logic
        $card = new Card("", "", "", "", "", "", "", "");

        $deleteCardId = $_POST['deleteCardId'];
        $success = $card->deleteCard($pdo, $deleteCardId);

        if ($success) {
            echo "Deleted";
        } else {
            echo "Deletion failed.";
        }
    }
}
